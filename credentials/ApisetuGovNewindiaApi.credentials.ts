import type {
	IAuthenticateGeneric,
	Icon,
	ICredentialType,
	INodeProperties,
} from 'n8n-workflow';

export class ApisetuGovNewindiaApi implements ICredentialType {
	name = 'N8nDevApisetuGovNewindiaApi';

	displayName = 'Apisetu Gov Newindia API';

	icon: Icon = { light: 'file:../nodes/ApisetuGovNewindia/apisetu-gov-newindia.svg', dark: 'file:../nodes/ApisetuGovNewindia/apisetu-gov-newindia.dark.svg' };

	documentationUrl = '';

	properties: INodeProperties[] = [
		{
			displayName: 'Base URL',
			name: 'url',
			type: 'string',
			default: 'https://apisetu.gov.in/newindia/v3',
			required: true,
			placeholder: 'https://apisetu.gov.in/newindia/v3',
			description: 'The base URL of your Apisetu Gov Newindia API server',
		},
		{
			displayName: 'API Key',
			name: 'apiKey',
			type: 'string',
			typeOptions: { password: true },
			default: '',
			required: false,
		},
		{
			"displayName": "❤️ Keep It Moving: One developer built a tool that auto-generates n8n nodes from any OpenAPI spec. Your donation funds new features, more API support, and better tooling for every developer after you. <a href=\"https://n8n-code.github.io/membership/#\" target=\"_blank\">☕ Buy me a coffee</a>",
			"name": "n8nCodeNotice",
			"type": "notice",
			"default": "",
			"displayOptions": {
				"show": {}
			}
		}
	];

	authenticate: IAuthenticateGeneric = {
		type: 'generic',
		properties: {
			headers: {
				'X-APISETU-APIKEY': '={{$credentials.apiKey}}',
			},
		},
	};


}
